...
import { FlatList, AppRegistry, View, Alert, Image, StatusBar, StyleSheet, Platform, Dimensions, TouchableOpacity } from "react-native";
import {
...
export default class MBHRAP002_3 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            checkboxes: [],
    ...
    componentDidUpdate(prevProps) {
        if (prevProps.data !== this.props.data) {
            ...
            this.props.data.map(item => {
                comment.push({ pk: item.pk, comment: item.approve_note });
            })
            this.setState({
                comment,
                numberRecordRender: 3,
    ...
    updateExperience = async (p_approve_status) => {
        ...
        dataSend.map((item, index) => {
            if (index + 1 == dataSend.length) {
                para += `${p_action}|${item.pk}|${p_approve_status}|${item.comment}|${this.state.dt_crt_by}`;
            } else {
                para += `${p_action}|${item.pk}|${p_approve_status}|${item.comment}|${this.state.dt_crt_by}*|*`;
            }
        })

        let procedure = "STV_HR_UPD_MBI_MBHRAP002_1";
            ...
            .then((res) => {
                ...
                if (data_info.totalrows > 0) {
                    if (data_info.records[0].status == "OK") {
                        isThis.setState({
                            loading: false, colorStatus: '#51bc8a',
                            status: 'OK',
                            comment: [],
                            checkboxes: [],
                            checkAll: false,
                        ...
                        variables.toastSuccessful.text = 'Không phê duyệt tăng ca thành công!'
                        Toast.show(toast)
                        this.calc();

                    } else {
                        isThis.setState({ loading: false, status: 'NOOK', colorStatus: 'red' });
                        if (data_info.records[0].error_ex != "") {
                            ...
                            let toast = variables.toastError;
                            toast.text = 'Không phê duyệt tăng ca thất bại!'
                            Toast.show(toast)
                            return;
                        }
    ...
    comfirmAlert(p_approve_status) {
        ...
        Alert.alert(
            'Không phê duyệt tăng ca',
            'Bạn có chắc chắn muốn không phê duyệt?',
            [
            ...
            <View style={{
                ...
                <View style={{ flexDirection: 'row', }}>
                    <Body style={{ borderColor: '#eeee', borderRadius: 5, borderWidth: 1, marginLeft: 5, }}>
                        ...
                        <View style={styles.cart}>
                            <Text style={styles.textCart}>Ca làm việc: </Text>
                            <Right>
                                <Text style={[styles.textCart1Value, { color: variables.textValue }]}>{item.shift}</Text>
                            </Right>
                            <View style={GlobalStyles.spaceHorizontalSmall}></View>
                        </View>

                        <View style={styles.cart}>
                            <Text style={styles.textCart}>Giờ vào - ra: </Text>
                            <Right>
                        ...
                        <View style={styles.cart}>
                            <Text style={styles.textCart}>Giờ làm việc: </Text>
                            <Right>
                                <Text style={[styles.textCart1Value, { color: variables.textValue }]}>{item.wt}</Text>
                            </Right>
                            <View style={GlobalStyles.spaceHorizontalSmall}></View>
                        </View>

                        <View style={styles.cart}>
                            <Text style={styles.textCart}>Giờ tăng ca: </Text>
                            <Right>
                                <Text style={[styles.textCart1Value, { color: variables.textValue }]}>{item.ot}</Text>
                            </Right>
                            <View style={GlobalStyles.spaceHorizontalSmall}></View>
                        </View>

                        <View style={styles.cart}>
                            <Text style={styles.textCart}>Giờ tăng ca xác nhận (NLĐ): </Text>
                            <Right>
                                <Text style={[styles.textCart1Value, { color: variables.textValue }]}>{item.ot_xn}</Text>
                            </Right>
                            <View style={GlobalStyles.spaceHorizontalSmall}></View>
                        </View>
                        ...
                        <View style={styles.cart, { flex: 1, flexDirection: "row" }}>
                            <TextInput1
                                ...
                                editable={false}
                                scrollEnabled={false}
                                value={!!item.reason_ot ? item.reason_ot : ' '}
                                theme={{
                                    colors: {
                ...
                {lengthDataProps > 0 ?
                    <View>
                        <View style={[GlobalStyles.row, { flex: 0, marginVertical: 2, paddingBottom: 0 }]}>
                            ...
                            <View style={[GlobalStyles.rowCenter, { marginLeft: '25%' }]}>
                                <Button
                                    icon={
                                        <Icon
                                            style={[GlobalStyles.iconHeader, { fontSize: 22, marginLeft: 5, marginBottom: '4%', height: '100%' }]}
                                            name="ios-checkmark"
                                        />
                                    }
                                    onPress={() => this.comfirmAlert(1)}
                                    iconContainerStyle={{ marginLeft: 15, paddingLeft: 15 }}
                                    buttonStyle={{ height: 33, backgroundColor: 'hsl(40, 89%, 52%)' }}
                                    raised
                                    iconRight
                                    loading={false}
                                    title="Chờ phê duyệt"
                            ...
                            <View style={[GlobalStyles.rowRight, { marginRight: '2%', }]}>
                                <CheckBox
                                    iconRight={true}
                                    center={true}
                                    title='Tất cả'
